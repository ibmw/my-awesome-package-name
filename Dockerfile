...
ENV VIRTUAL_ENV /opt/my-awesome-package-name-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/my-awesome-package-name/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/my-awesome-package-name/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/my_awesome_package_name/ && touch src/my_awesome_package_name/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/my-awesome-package-name/
